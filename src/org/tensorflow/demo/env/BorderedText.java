...
public class BorderedText {
  ...
  public BorderedText(final int interiorColor, final int exteriorColor, final float textSize) {
    ...
    interiorPaint.setTextSize(textSize+5);
    interiorPaint.setColor(exteriorColor);
    interiorPaint.setStyle(Style.FILL);
    interiorPaint.setAntiAlias(false);
    interiorPaint.setAlpha(255);

    exteriorPaint = new Paint();
    exteriorPaint.setTextSize(textSize+5);
  ...
  public void drawText(final Canvas canvas, final float posX, final float posY, final String text,int color) {

    Paint mpaint= new Paint();
    mpaint.setColor(color);
    mpaint.setStyle(Style.FILL);
    float w = interiorPaint.measureText(text)/2;
   canvas.drawRect(posX-15, posY - textSize-10, posX+w + w+25, posY+10, mpaint);
   // canvas.drawText(text, posX, posY,exteriorPaint );
    canvas.drawText(text, posX, posY-5, interiorPaint);
  }
  public void drawText1(final Canvas canvas, final float posX, final float posY, final String text,int color) {
  ...
  public void drawLines(Canvas canvas, final float posX, final float posY, Vector<String> lines) {
    ...
    for (final String line : lines) {
      drawText(canvas, posX, posY - getTextSize() * (lines.size() - lineNum - 1), line,0);
      ++lineNum;
    }
  }
